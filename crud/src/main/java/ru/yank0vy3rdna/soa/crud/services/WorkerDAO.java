package ru.yank0vy3rdna.soa.crud.services;
...
import ru.yank0vy3rdna.soa.crud.utils.FilterParser;
import ru.yank0vy3rdna.soa.crud.utils.SortByParser;
...
public class WorkerDAO {
    ...
    public Worker getWorkerById(Session session, Long workerId) {
        return (Worker) session.createQuery("from worker where id=:id").setParameter("id", workerId).getSingleResultOrNull();
    }

    public Worker getWorkerWithMaxOrg(Session session) {
        return session.createNativeQuery("select * from workers order by organization->>'fullName' desc limit 1", Worker.class).getSingleResultOrNull();
    }
